		...
		if (
			...
			if (reason.toLowerCase().includes("sponsor")) {
				throw new SponsorRefusedError(
					`The sponsor rejected the transaction: ${reason}`,
					reason,
				);
			}
			throw new TransactionRejectedError(
				`The engine rejected the transaction: ${reason}`,
				reason,
			);
		}
	...
	async getTransactionState(jobId: string): Promise<JobState | null> {
		...
		try {
			response = await fetch(
				this.resolveUrl(`/transaction/${encodeURIComponent(jobId)}`),
				{
					signal: AbortSignal.timeout(this.requestTimeoutMs),
				},
			);
		...
		if (!response.ok) {
			const reason = await this.errorBody(response);
			throw new NodeUnreachableError(
				`Could not read job ${jobId}: ${reason}`,
				reason,
			);
		}
	...
	async broadcastFrameTransaction(
		...
		while (Date.now() < deadline) {
			...
			if (state?.status === "failed" || state?.status === "superseded") {
				...
				throw new TransactionRejectedError(
					`The transaction was not broadcast: ${reason}`,
					reason,
				);
			}
